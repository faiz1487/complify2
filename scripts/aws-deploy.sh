...
echo "==> Step 1: Provision AWS infrastructure (VPC, RDS, Redis, S3, ECR, ALB, ECS)"
...
aws ecs update-service --cluster "$CLUSTER" --service ingestion-service --force-new-deployment --region "$REGION" >/dev/null
aws ecs update-service --cluster "$CLUSTER" --service retrieval-service --force-new-deployment --region "$REGION" >/dev/null

echo "    Waiting for ECS services to stabilize..."
aws ecs wait services-stable --cluster "$CLUSTER" --services ingestion-service retrieval-service --region "$REGION"
