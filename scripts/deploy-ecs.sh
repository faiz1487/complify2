...
UPLOAD_SERVICE="${UPLOAD_SERVICE:-upload-service}"
PROCESSING_SERVICE="${PROCESSING_SERVICE:-data-processing-service}"
...
aws ecs wait services-stable \
  --cluster "$CLUSTER_NAME" \
  --services "$UPLOAD_SERVICE" "$PROCESSING_SERVICE" "$RETRIEVAL_SERVICE" \
  --region "$AWS_REGION"
