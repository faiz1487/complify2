  ...
  container_definitions = jsonencode([
    {
      ...
      healthCheck = {
        command     = ["CMD-SHELL", "python -c \"import urllib.request; urllib.request.urlopen('http://127.0.0.1:4001/health')\" || exit 1"]
        interval    = 30
        timeout     = 5
        retries     = 3
        startPeriod = 60
...
resource "aws_ecs_service" "ingestion" {
  ...
  desired_count   = var.app_ready ? var.ingestion_desired_count : 0
  launch_type     = "FARGATE"
