...
resource "aws_lb_listener" "http" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "{\"message\":\"Document API\",\"upload\":\"POST /api/upload\",\"search\":\"GET /api/search/{filename}\",\"health_upload\":\"GET /health/upload\",\"health_retrieval\":\"GET /health/retrieval\"}"
      status_code  = "200"
    }
  }
}

resource "aws_lb_listener_rule" "health_upload" {
  ...
  condition {
    path_pattern {
      values = ["/health/upload", "/health/upload/*"]
    }
  }
}
...
resource "aws_lb_listener_rule" "upload" {
  ...
  condition {
    path_pattern {
      values = ["/api/upload", "/api/upload/*", "/docs*", "/openapi.json"]
    }
  }
}
...
resource "aws_lb_listener_rule" "search" {
  ...
  condition {
    path_pattern {
      values = ["/api/search", "/api/search/*", "/api/documents/*"]
    }
  }
}
